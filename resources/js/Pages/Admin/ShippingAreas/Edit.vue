...
const { form, processing, errors, editAction } = useResourceActions({
  region_id: props.shippingArea?.region_id,
  city_id: props.shippingArea?.city_id,
  township_id: props.shippingArea?.township_id,
  name: props.shippingArea?.name
})
...
<template>
  <Head :title="__('Edit :label', { label: __('Shipping Area') })" />
    ...
    <div class="min-h-screen py-10 font-poppins">
      ...
      >
        <Breadcrumb to="admin.shipping-areas.index" icon="fa-location-dot" label="Shipping Areas">
          <BreadcrumbItem :label="shippingArea.name" />
      ...
      <div class="border p-10 bg-white rounded-md">
        <form
          @submit.prevent="
            editAction('Shipping Area', 'admin.shipping-areas.update', {
              shipping_area: shippingArea?.slug
            })
          "
