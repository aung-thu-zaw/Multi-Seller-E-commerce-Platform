...
const props = defineProps({ storeProductBanner: Object })

const { previewImage, setImagePreview } = useImagePreview(props.storeProductBanner?.image)

const { form, processing, errors, editAction } = useResourceActions({
  url: props.storeProductBanner?.url,
  status: props.storeProductBanner?.status,
  logo: props.storeProductBanner?.logo
})
</script>
    ...
    <div class="min-h-screen py-10 font-poppins">
      ...
      >
        <Breadcrumb to="seller.store-product-banners.index" icon="fa-ad" label="Product Banners">
          <BreadcrumbItem :label="storeProductBanner.url" />
      ...
      <div class="border p-10 bg-white rounded-md">
        <form
          @submit.prevent="
            editAction('Product Banner', 'seller.store-product-banners.update', {
              store_product_banner: storeProductBanner?.uuid
            })
          "
