...
<template>
  <Head :title="__(':label Dashboard Login', { label: __('Seller') })" />
    ...
    <AuthFormContainer>
      ...
      <form @submit.prevent="submit" class="w-full space-y-4 md:space-y-6">
        <h1 class="text-center text-2xl text-dark mb-5 font-bold">
          {{ __(':label Dashboard Login', { label: __('Seller') }) }}
        </h1>
        ...
        <div class="flex items-center justify-between mb-5">
          ...
          <div>
            <Link
              ...
              class="underline text-sm text-gray-600 rounded-md hover:text-blueGray-500"
            >
              {{ __('Forgot Password ?') }}
            </Link>
          </div>
