...
onMounted(() => {
  ...
  Echo.private(`App.Models.User.${usePage().props.auth.user?.id}`).notification((notification) => {
    if (notification.type === 'App\\Notifications\\Seller\\NewProductQuestionNotification') {
...
<template>
  <div class="hs-dropdown relative inline-flex [--trigger:hover] [--placement:bottom-right]">
    ...
    >
      ...
      >
        <span class="font-bold text-blue-500 text-[.6rem]">
    ...
    >
      <div class="px-4 py-3 bg-gray-100 flex items-center justify-between border-b w-full">
        ...
        <button
          ...
          type="button"
          class="text-blue-600 hover:text-blue-500 text-sm font-bold"
        >
          {{ __('Mark all as read') }}
        </button>
      </div>
      ...
      <div v-if="notifications.length" class="w-full">
        <div v-for="notification in sortedNotifications" :key="notification.id" class="w-full">
          <!-- Notification Cards -->
          <ProductQuestionNotificationCard
            v-show="
